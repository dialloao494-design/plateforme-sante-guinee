...
| 2026-08-31 | Platform and clinic administration now share one transactional staff-lifecycle service with immediate refresh/access-token invalidation, membership updates, last-administrator and clinical-history guards, and reasoned before/after audit evidence. A platform control room adds classified test/technical/production accounts, guarded bulk cleanup previews, searchable staff/security views, clinic configuration and suspension workflows, operational/offline/backup health, searchable CSV/PDF audit exports, duplicate-patient merge preview/execution, scoped data export, and backup-gated typed-confirmation clinic reset. The migration is compatible with pristine and partially stamped databases. Local evidence: backend **469 passed / 1 skipped**, platform governance focus **19/19**, migration focus **15/15**, frontend/domain **51/51**, offline **45/45**, lint and production PWA build pass. | Administration governance is code-complete and locally regression/build verified. Remote CI, deployment, production migration/health verification, restore drill, observed administrator usability review, MFA enforcement/provider integration, and field validation remain open; persisted module/payment configuration is an administration foundation and does not itself prove every clinical screen enforces each setting. |
| 2026-08-31 | Administration follow-through now keeps daily sign-in simple while strengthening sensitive support workflows: platform administrators can inspect active staff sessions, revoke them, and send auditable secure password-reset links instead of assigning shared temporary credentials. Clinic security configuration records an administrator-focused MFA policy and trusted-workstation duration without forcing MFA on ordinary clinical staff. Billing now rejects payment methods disabled by clinic configuration with a plain corrective message. Operational health reads the actual database migration revision and exposes workstation storage and license signals when reported. Local regression remains green: backend **469 passed / 1 skipped**, frontend/domain **51/51**, offline **45/45**, lint, production PWA build, and all six performance budgets pass. | Code-complete and locally regression/build verified. The security policy is deliberately progressive to avoid locking out low-digital-literacy staff. CI/deployment, production signal validation, selective single-session revocation UI, observed clinic-admin usability, restore drills, and enforcement of every module toggle across every clinical route remain open. |
| 2026-08-31 | Clinic and platform administration now expose one explicit staff-edit workflow for first name, last name, and role. Changes require a reason and record before/after audit evidence; permission changes immediately revoke sessions and increment token versions. Clinic administrators cannot grant administrator privileges or change their own role, the final active clinic administrator cannot be demoted, and platform administrators use the same lifecycle service. Existing doctor profiles are kept name-consistent. The former platform live role dropdown is replaced by a deliberate save action, while the clinic directory provides a responsive, clearly labeled edit panel. Local evidence: backend **472 passed / 1 skipped**, focused governance **16/16**, frontend/domain **51/51**, lint, production PWA build, and diff validation pass. | Staff editing is code-complete and locally regression/build verified. Remote CI, deployment, production verification, and observed administrator usability remain open. |
| 2026-09-01 | A clinic workstation remained trapped in offline mode after its internet connection returned because the browser's `navigator.onLine` value stayed stale. While offline or degraded, the client now probes the platform's own health endpoint every eight seconds and when the window regains focus or visibility. A successful first-party probe restores application connectivity and immediately retries the durable outbox; only that proven server response may bypass the stale browser flag. Local evidence: offline unit suite **45/45**, exact Chromium stale-connectivity recovery **1/1**, frontend lint, and production PWA build pass. The public production health endpoint was independently reachable during diagnosis. | Reconnection recovery is code-complete and locally unit/browser/build verified. CI, deployment, exact production-build browser verification, and confirmation on the affected physical clinic workstation remain open; production API health alone does not prove field recovery. |
| 2026-09-01 | The pharmacy defect review produced a connected operational ledger rather than isolated counters. Pharmacy now has explicit Commandes, Dispensation, Stock, Historique, and Rapport sections; replenishment orders persist medicine, quantity, supplier, lifecycle, actor, and receipt time, and receiving once increases stock transactionally while repeat receipt is rejected. Supplier is removed from the dispensing stock table and remains in the procurement register. Completed dispensations remain searchable with stable `PHARM-…` request numbers. Monthly reporting derives distinct patient volume, requests, dispensations, generated/collected/pending GNF, collection rate, and top medicines from tenant-scoped orders, charges, and payment rows. The shared server invoice template adds a self-contained invoice-identification QR. Secondary panels are lazy-loaded to preserve the daily route budget. Local evidence: backend **473 passed / 1 skipped**, frontend/domain **51/51**, offline **45/45**, migration/invoice focus **25/25**, pharmacy inventory/clinical focus **6/6**, exact responsive Chromium **1/1**, lint, production PWA build, and all six performance budgets pass. | Pharmacy operations/reporting is code-complete and locally regression-, migration-, responsive-browser-, offline-, build-, and performance-verified. CI, deployment/migration, production smoke/data verification, physical invoice scan validation, and observed pharmacist acceptance remain open; the QR identifies the printed invoice and is not yet a public verification portal. |
| 2026-09-02 | The remaining pharmacy field requests were implemented as one guided ledger extension. Dispensation now presents an explicit patient → saved service request → payment sequence and hides payment controls until the tenant-scoped request and charge exist. Dispensed history exposes a named invoice reprint action rather than treating the status badge as a control. A dedicated reimbursement workspace searches only paid/refundable pharmacy charges, limits quantities and value against prior refunds, records reason/recipient/method/actor, can return confirmed reusable products to stock, emits an official PDF receipt, audits the operation, and feeds refunded/net revenue into the monthly report. A second-pass accounting review corrected the refund transaction so the original payment remains a gross paid receipt, the refund is its separate offsetting ledger, cumulative consumption prints correctly, and the refund plus audit evidence commit atomically. Local evidence: backend **474 passed / 1 skipped**, frontend/domain **51/51**, offline **45/45**, focused pharmacy/reporting **4/4**, focused responsive Chromium **1/1**, lint, production PWA build, and all six performance budgets pass. | Pharmacy request-before-billing, historical invoice reprint, and guarded refund flows are code-complete and locally regression-, responsive-browser-, offline-, build-, accounting-integrity-, and performance-verified. CI, deployment/migration, production smoke/data verification, physical printing, and pharmacist field validation remain open. |
| 2026-09-02 | The Guinea-only platform-owner console was reorganized around international SaaS operational standards without adding unnecessary multi-country complexity. The owner now lands on an exception-first network command center; distinct Operations and Security destinations replace the duplicated System/Settings screen; production-clinic activity, missing administrators, offline sync backlog/conflicts, workstation visibility, backup verification, database status, MFA gaps, lockouts, failed logins, and active sessions have guided drill-downs into existing governed clinic/account workflows. Owner navigation, breadcrumbs, mobile layouts, keyboard focus, actionable empty/error states, and French terminology were aligned. A second adversarial pass found and corrected an owner-only back-link exposed in the delegated platform-administrator clinic directory, then locked the role separation with a direct navigation regression. Local evidence: frontend **52/52**, offline **45/45**, focused platform governance backend **29/29**, full backend **475 passed / 1 skipped**, complete Chromium matrix **54/54**, lint, production PWA build, and all six performance budgets pass. | Platform-owner UX overhaul is code-complete and locally accessibility-, responsive-browser-, cross-role-, offline-, backend-, build-, and performance-verified. CI, deployment, production-data verification, incident/restore exercises, and observed platform-owner field acceptance remain open; this does not claim that unavailable deployment rollback or external monitoring controls exist. |
| 2026-09-02 | Platform ownership was changed from a single-account constraint to a resilient multi-owner model. The legacy partial unique index is removed by Alembic; trusted provisioning can create additional owners; and the public first-owner setup remains permanently closed after the first owner exists. Regression coverage verifies both behaviors. Local evidence: full backend **476 passed / 1 skipped**, focused owner/migration **23/23**, Python compilation, Alembic single-head, and clean diff checks pass. | CODE COMPLETE and locally verified. CI, production migration, and creation of the requested additional owner remain open. |
| 2026-09-02 | Hospitalization billing now reflects the five confirmed daily tariffs: pediatric cradle **80,000 GNF**, pediatric bed **120,000 GNF**, shared-room bed **180,000 GNF**, shared-room bed **200,000 GNF**, and VIP cabin **500,000 GNF**. Reception shows the selected daily rate, number of days, and calculated total before submission; the authoritative server multiplies the catalog price by days, and online/offline invoice views print the quantity as `jour(s)`. Browser printing now isolates the invoice document in a dedicated print frame so the hidden application shell cannot create trailing blank pages. Local evidence: backend **475 passed / 1 skipped**, focused hospitalization/billing **16/16**, frontend/domain **51/51**, offline **45/45**, exact responsive service-request Chromium **2/2**, lint, production PWA build, and all six performance budgets pass; generated-invoice regression asserts one nonblank PDF page for the new three-day tariff case. | Hospitalization tariff/day billing and invoice pagination corrections are code-complete and locally regression-, responsive-browser-, offline-, PDF-, build-, and performance-verified. CI, deployment, production smoke/data verification, physical print-preview validation on clinic printers, and reception field acceptance remain open. |
| 2026-09-02 | CI run **33658964802** correctly rejected the hospitalization tranche in `backend-tests`, `clinic-regression-gates`, and `e2e-tests`. The two Python jobs lacked the newly used `pypdf` test dependency in the locked development environment. The browser gate also exposed a real first-offline-print defect: the isolated print helper was requested only after connectivity was lost, and the prior test still assumed whole-page printing. `pypdf` is now locked for development/CI, Reception warms the separately bundled helper while online, and the offline regression verifies the isolated print boundary and that the application document remains excluded from print. Local CI-mode evidence after correction: formerly failing offline receipt case **1/1**, focused hospitalization/billing **16/16**, lint/build/performance budgets pass with Reception at **114.98/115 KiB**. | The failed CI run remains historical failure evidence and is not relabeled green. Corrections are locally CI-mode verified; a new remote CI run must pass all required jobs before this tranche becomes CI verified or deployable. |
| 2026-09-09 | Production login diagnosis found that only mutable counters and transient application logs existed for authentication outcomes. Migration `0040` adds an append-only `authentication_events` ledger containing user/clinic IDs when known, bounded outcome/reason codes, an HMAC email fingerprint, IP, user agent, request ID, and timestamp; it never stores raw email, submitted passwords, password hashes, MFA codes, or tokens. Login outcomes now cover unknown users, invalid credentials/hashes, lockout, disabled/unverified accounts, MFA denial/enrollment requirements, and completed success. A correct password followed by failed MFA no longer updates `last_login_at` as a successful login. Focused authentication regression passes **19/19**, and a clean SQLite migration reaches the single `0040` head. | Authentication observability is code-complete and locally regression/migration verified. CI, production migration, retention-policy approval, production log review, and administrator-facing governed query/export remain open. |
| 2026-09-09 | Staff deletion policy is now explicit and consistent across clinic and platform administration. The backend computes one authoritative eligibility decision: only inactive, valid, never-used invitations without login, verification, doctor-profile, or audit history can be permanently deleted. Eligible rows expose “Supprimer définitivement”; every retained identity displays a specific French explanation and remains available for deactivation/reactivation. The delete service reuses the same decision, preserving self-delete, tenant, session, clinical-attribution, and audit safeguards. Focused governance/lifecycle tests pass **18/18**; frontend unit tests pass **55/55** and lint is green. | Guarded staff deletion UX is code-complete and locally unit/regression verified. CI, deployment, production verification, and observed administrator acceptance remain open. |
| 2026-09-09 | The clinic-supplied AASMA plastic-surgery tariff was transcribed into a clinic-17-only catalogue of **102** coded services across Générale, Reconstruction, Esthétique, and Brûlés. Four document entries without printed codes use approved internal codes; the two clinically distinct spontaneous-healing hospitalization choices remain separately visible at **50,000 GNF** (`QAASMA-HospCD`) and **150,000 GNF** (`QAASMA-HospCD2`). Catalogue composition reconciles by code, the physician selector exposes section/code/tariff, and service-request/invoice resolution re-resolves the clinic-scoped authoritative price server-side while retaining surgical codes in printed line descriptions. Focused backend workflow/catalogue tests pass **21/21**; frontend unit tests pass **55/55**, with lint, production PWA build, and all performance budgets green. | AASMA plastic-surgery catalogue integration is code-complete and locally regression/build verified. CI, deployment, production catalogue verification, and physician/Reception field validation remain open; prices become operational only after deployment. |
| 2026-09-09 | Native browser constraint validation could stop form submissions before page handlers ran, making actions appear inert—especially when the invalid field was inside a collapsed section. Application-wide interaction feedback now identifies, reveals, centers, and focuses the first invalid field, while pointer/touch attempts on disabled buttons explain that the action is unavailable (with an optional workflow-specific `data-disabled-reason`). The Reception registration form is separately loaded to preserve route performance. Chromium verifies both hidden-invalid-field and disabled-action feedback within the registration file **4/4**, and the complete cross-role browser matrix passes **55/55**. Frontend tests pass **56/56**, offline tests **45/45**, lint/build and all budgets pass, with Reception at **94.59/115 KiB**. The same review ran the full backend at **483 passed / 1 skipped** and a clean migration through head `0040`. | Shared interaction feedback is code-complete and locally unit-, full-surface real-browser-, offline-, build-, performance-, and backend-regression-verified. CI, deployment, production-log correlation, and confirmation across representative clinic workstations remain open. |
