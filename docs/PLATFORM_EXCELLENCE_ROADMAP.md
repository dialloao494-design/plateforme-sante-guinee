...
| 2026-09-09 | Reception’s long patient-registration form could appear inert when native browser constraint validation stopped submission before the React handler, particularly when an invalid optional field was inside the collapsed complementary-information section. The first invalid field is now revealed, centered, focused, and named in an error beside the sticky submit action; the registration form is separately loaded to keep the Reception route below budget. The exact hidden-invalid-email Chromium regression passes **1/1** within the full registration file **4/4**; frontend tests pass **56/56**, lint/build pass, and the Reception route is **94.67/115 KiB**. The same review ran the full backend at **483 passed / 1 skipped** and a clean migration through head `0040`. | Registration feedback correction is code-complete and locally unit-, real-browser-, build-, performance-, and backend-regression-verified. CI, deployment, production-log correlation, and confirmation on the affected Reception workstation remain open. |
