...
export default function PlatformClinicDetail() {
  ...
  const examineStaff = (member) => {
    setSelectedStaff(member);
    setStaffDraft({ first_name: member.first_name || '', last_name: member.last_name || '', role: member.role, reason: 'Mise à jour par l’administration plateforme' });
  };

  const saveStaff = async (event) => {
    event.preventDefault();
    if (!selectedStaff || !staffDraft) return;
    setBusy(true); setError('');
    try { const { data } = await platformApi.updateStaff(id, selectedStaff.id, staffDraft); setMessage(`Fiche de ${selectedStaff.email} mise à jour.`); setSelectedStaff(data); setStaffDraft({ ...staffDraft, first_name: data.first_name || '', last_name: data.last_name || '', role: data.role }); await loadAll(); }
    catch (err) { setError(formatApiError(err, 'Modification du personnel impossible.')); } finally { setBusy(false); }
  };
  ...
  return <main className="clinical-page platform-clinic-detail" id="main-content">
    ...
    {tab === 'staff' && <section className="clinical-card platform-control-card--wide"><div className="platform-section-heading"><div><p className="clinical-eyebrow">Annuaire opérationnel</p><h2>Personnel</h2></div><span>{visibleStaff.length} résultat(s)</span></div>
      ...
      <div className="platform-staff-layout"><div className="users-table-wrap"><table className="users-table"><thead><tr><th>Identité</th><th>Rôle</th><th>Accès</th><th>Dernière connexion</th><th>Actions</th></tr></thead><tbody>{pageRows.map((u) => <tr key={u.id} className={!u.is_active ? 'platform-row-inactive' : ''}><td><strong>{u.full_name || u.email}</strong><span className="platform-table-sub">{u.email}</span></td><td>{ROLE_LABELS[u.role] || u.role}</td><td>{u.is_active ? 'Actif' : u.invitation_status ? 'Invitation en attente' : 'Inactif'}</td><td>{formatDate(u.last_login_at)}</td><td><button type="button" onClick={() => examineStaff(u)}>Examiner</button></td></tr>)}</tbody></table></div>{selectedStaff && staffDraft && <aside className="platform-staff-drawer"><button type="button" className="platform-drawer-close" aria-label="Fermer le détail" onClick={() => { setSelectedStaff(null); setStaffDraft(null); }}>×</button><p className="clinical-eyebrow">Compte #{selectedStaff.id}</p><h3>{selectedStaff.full_name || selectedStaff.email}</h3><dl><dt>E-mail</dt><dd>{selectedStaff.email}</dd><dt>Créé</dt><dd>{formatDate(selectedStaff.created_at)}</dd><dt>Dernière connexion</dt><dd>{formatDate(selectedStaff.last_login_at)}</dd><dt>Sessions actives</dt><dd>{selectedStaff.active_sessions}</dd><dt>MFA</dt><dd>{selectedStaff.mfa_enabled ? 'Activée' : 'Non activée'}</dd><dt>Verrouillage</dt><dd>{selectedStaff.locked_until ? `Jusqu’au ${formatDate(selectedStaff.locked_until)}` : 'Aucun'}</dd></dl><form className="platform-staff-edit" onSubmit={saveStaff}><label>Prénom<input required value={staffDraft.first_name} onChange={(e) => setStaffDraft({ ...staffDraft, first_name: e.target.value })} /></label><label>Nom<input required value={staffDraft.last_name} onChange={(e) => setStaffDraft({ ...staffDraft, last_name: e.target.value })} /></label><label>Rôle<select value={staffDraft.role} disabled={busy} onChange={(e) => setStaffDraft({ ...staffDraft, role: e.target.value })}>{STAFF_ROLES.map((role) => <option key={role} value={role}>{ROLE_LABELS[role] || role}</option>)}</select></label><label>Raison<input required minLength="3" value={staffDraft.reason} onChange={(e) => setStaffDraft({ ...staffDraft, reason: e.target.value })} /></label><button type="submit" className="clinical-btn" disabled={busy}>{busy ? 'Enregistrement…' : 'Enregistrer les modifications'}</button></form><div className="platform-drawer-actions"><button type="button" onClick={() => showDialog('sessions', selectedStaff)}>Déconnecter toutes les sessions</button><button type="button" onClick={() => showDialog(selectedStaff.is_active ? 'deactivate' : 'reactivate', selectedStaff)}>{selectedStaff.is_active ? 'Désactiver' : 'Réactiver'}</button>{selectedStaff.can_delete ? <button type="button" className="platform-danger-link" onClick={() => showDialog('delete', selectedStaff)}>Supprimer définitivement</button> : <span className="platform-table-sub">{selectedStaff.delete_blocked_reason || 'Cette identité doit être conservée pour la traçabilité.'}</span>}</div></aside>}</div>
