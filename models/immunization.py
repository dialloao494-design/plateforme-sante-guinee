...
class ImmunizationRecord(Base):
    ...
    vaccine_expiry_date = Column(Date, nullable=True)
    injection_site = Column(String(64), nullable=True)
    vaccination_strategy = Column(String(32), nullable=True, default="routine")
    age_at_vaccination_months = Column(Integer, nullable=True)
    age_at_vaccination_days = Column(Integer, nullable=True)
    aefi_notes = Column(Text, nullable=True)
    administered_at = Column(Date, nullable=False)
    next_appointment_date = Column(Date, nullable=True)
    vaccinator_name = Column(String(128), nullable=True)
